...
export const desc = 'Show information about translations in your project';
...
export const builder = (yyargs: yargs.Argv) => yyargs
    .usage(outdent`
        ...
        Shows information about the translations in your project.
        Indicates if you should run "riw project translate".
    `)
    .options({
        missing: {
            alias: 'm',
...
export const handler = createHandlerWithRIW((riw: RIW) => {
    ...
    const {
        ...
        dateDB,
        dateConfig,
    }: RIWCLIProjectStatusResult = riw.project.status();

    if (ctDefault == null) {
        spinner.fail(outdent`
            No locale output file(s) found. Run "riw project translate".
        `);
        return;
    }
    ...
    if (todo == null) {
        spinner.fail(outdent`
            No todo file found. Run "riw project translate".
        `);
        return;
    }
    ...
    if (isShowingWarning) {
        spinner.fail(outdent`
            Run ${chalk.bold('riw project translate')} to regenerate output files.
        `);
    }
